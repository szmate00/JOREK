...
subroutine state_gauss_minima(my_id, node_list, element_list, vars, minima)
  ...
  real*8  :: eq_g(n_plane, n_gauss, n_gauss), local(size(vars))
  ...
  do ife = ife_min, ife_max
    ...
    do kv = 1, size(vars)
      ...
      do i = 1, n_vertex_max
        ...
        enddo
      enddo
      local(kv) = min(local(kv), minval(eq_g))
    enddo
  enddo
