...
module phys_module
  ...
  logical :: bohm_drift_compatible !< Impose the DRIFT-COMPATIBLE Bohm condition on vpar, v_par = sigma*c_s - (v_ExB.n)/b_n, i.e. the total normal velocity reaches the Bohm flux rather than the parallel part alone. The drift term is already present in Mach1BC but is divided by Btot a second time: R^2*d_b(u)/d_b(psi) equals -(v_ExB.n)/(b_n*Btot), so it is ALREADY in Vpar units (Vpar = v_par/|B|) and needs no conversion, whereas cs0 is a physical speed and genuinely does. The result is a drift correction short by a factor |B| ~ 2. Matches SOLPS-ITER BCMOM=13, which applies the ExB at coefficient 1 (b2stbc_cbc = 1.0). See doc/jorek_vpar_bc_drifts.pdf and doc/drift_bc_audit.pdf
  real*8  :: bohm_drift_clip       !< Bound on the drift correction in units of c_s, mirroring SOLPS's restriction of the poloidal ExB to +-2*c_s*|b_x|. Needed rather than optional: the correction goes as 1/b_n and the diag_mach1 diagnostic measured it exceeding c_s on ~0.9 percent of the boundary weight, where it would reverse v_par. Only used when bohm_drift_compatible = .true.
