...
module phys_module
  ...
  real*8  :: sheath_wall_vel      !< Thin resistive wall: speed at which poloidal flux is let through the boundary, dpsi/dt = -sheath_wall_vel*(dpsi/dn - its value at t_start). The ExB flow at the wall drags flux against a frozen psi and it piles up in a layer of width eta/v_n, some 10 microns at Spitzer resistivity against a millimetre mesh; this lets the wall pass flux at the rate the flow delivers it. Expect the useful range around v_n = R du/dl at the target. 0 = frozen psi, i.e. the previous behaviour. NOTE: expressed per degree-of-freedom unit of dpsi/dn, so it is mesh dependent - scan it
  logical :: sheath_u_align_psi   !< Impose [u,psi] = 0 on the sheath boundary types, i.e. force the ExB flow at the wall to run along flux surfaces. The normal derivative of u is otherwise free and generally not flux aligned, and the resulting flux dragging has to be balanced by a resistive current ~[u,psi]/eta, which at Spitzer resistivity is enormous and forms a boundary layer of width eta/v_n far below the cell size. Replaces the vorticity-equation row at u's normal-derivative degree of freedom
  logical :: sheath_u_value_only  !< Impose the sheath characteristic on the NODE VALUE of u only, leaving u's tangential-derivative degree of freedom to the vorticity equation. The derivative row otherwise slaves du/dl to the tangential-derivative DOF of Te, and du/dl is v_E.n, the velocity that drags poloidal flux - so grid-scale noise in dTe/dl is converted directly into flux dragging with no dissipation anywhere in the path. Collocating the value keeps u = u_target at the nodes and lets the equation smooth in between
  ...
  real*8  :: sheath_zj_ratio_max  !< Validity gate for bcs%sheath_zj. Where the plasma demands |zj/zj_sat| far above 1 the sheath cannot carry that current at any potential, so the characteristic has no useful solution and u is driven without bound. Measured: a point whose j_sat collapsed drove the demand 5 -> 265 while u chased it to 500*Te. This fades the constraint out with weight 1/(1+(ratio/sheath_zj_ratio_max)**4), handing such nodes back to the frozen-zj baseline, which is stable. Smooth, and 1 to within 6% for ratio < 0.5*max. ~2 is a reasonable choice; 0 disables the gate. Treat it as a validity limit with a convergence knob: raise it and check the answer does not move
  ...
  real*8  :: sheath_sat_slope_e   !< Electron-side mirror of sheath_sat_slope, and the fix for TWO sheath surfaces. With one sheath type the wall potential is anchored by the Dirichlet gauge on the others; with two, u closes a circuit between them whose only impedance is the sheath, and on the electron branch sheath_x_limited's dxlim_dx = 1/(1+exp(z)) -> 0, so the row keeps no u column and nothing damps the loop. This blends the clamped exponent back towards the raw one, x_lim := x_lim + s_e*(X - x_lim), so dxlim_dx -> s_e /= 0 as X -> -inf and the row ALWAYS retains a u column. 0 = previous behaviour, bit-identical. Same status as the ion-side slope: a regularisation with a convergence knob - report at the smallest s_e that runs and show the answer does not move.
  real*8  :: sheath_weak_wmin    !< Minimum mean validity weight W_a = D_a/D0_a for a weak sheath row to be written, where D0_a is the UNWEIGHTED int N_a N_a dS. The obliqueness/ratio weight wk_wgt multiplies D_a, F_a and every J_ab identically, and the row is written as J/D and F/D, so a wk_wgt that is uniform over a row's support CANCELS EXACTLY: the row it was meant to fade out is written at full stiffness. That is why a surface whose whole support sits outside the characteristic's validity range (the divertor leg-end faces) diverges while a surface that is gated only in patches (the near-tangential wall, 42 % gated) is fine - a weight that VARIES within a row does not cancel. Row replacement with zbig is binary, so the fade has to be the decision whether to write the row at all, and W_a in [0,1] is the scale-free, mesh- and rank-independent measure of it. 0 = write every row, the previous behaviour.
  real*8  :: sheath_weak_detmin  !< Minimum node-frame determinant |x(1,2,:) x x(1,3,:)| for a weak sheath row to be written. The two first-derivative DOF directions at a node are not required to be orthogonal or even independent, and in the ray-cast wall extension (grids/grid_xpoint_wall.f90:1252 and :1311) they are the ray to the wall and an interpolated along-wall angle, which become PARALLEL where the extension meets the wall at grazing incidence. The determinant is |sin(angle between them)|, so the nodal derivative basis is conditioned as 1/det and zj = Delta*psi is built from SECOND derivatives. MEASURED on the ITER-like grid: det mean 0.88 / 0.87 / 0.38 / 0.05 and max 1/det 1.4 / 12.3 / 53.5 / 108.7 on boundary types 1 / 5 / 4 / 9, against 3900 / 308 / 8 / 4 steps survived - the ranking is exact. All 51 degenerate nodes sit at the two leg-end corners, with types 4, 5 and 9 interleaved inside each; the INNER corner is 1.65x worse than the outer, and every failure recorded for this boundary condition is on the inner target. Type 1 has 0 % of its nodes below 0.3, which is why type 1 alone never writes a row on a degenerate node and runs to timeout. UNLIKE sheath_weak_wmin this gate keys on a STATIC property of the mesh: it is fixed from step 1, identical on every rank, and the node set it removes is knowable before the run, so it cannot close in response to the divergence it is meant to prevent. It does NOT restore a boundary condition on u at the nodes it gates - on a weak type dirichlet%u and natural%u are both required .false. - so a type with a large gated fraction may fail for that reason instead. 0 = write every row, the previous behaviour, bit-identical.
  logical :: sheath_psi_jacobian  !< Add the psi column d(zj_sh)/d(psi) to the weak sheath row. zj_sh is proportional to g_bn/|B| and at the wall |B| = sqrt(F0^2+|grad psi|^2)/R, so zj_sh depends on psi through the field geometry. That column was omitted on the grounds that the geometry is frozen 'within a Newton iteration' (mod_sheath_trace header, and doc/sheath_bc_whitepaper.tex which calls it the most plausible remaining source of stiffness and the first thing to add if convergence degrades). There IS no Newton iteration: core/mod_jorek_timestepping.f90 does ONE construct_matrix and ONE solve_sparse_system per step with no loop, so the omitted term is a systematic per-step error rather than an iteration-count cost, and it compounds. dirichlet%psi pins psi's value and TANGENTIAL derivative but leaves the NORMAL derivative free, so the live columns are the direction_perp DOFs and the term is nonzero. Default .false. = previous behaviour, bit-identical.
  real*8  :: sheath_v_perp       !< Cross-field contribution to the ion saturation current, in JOREK velocity units: j_sat = e*n*(c_s*g(b_n) + v_perp). The parallel flux vanishes where the field grazes the wall, but perpendicular transport still delivers particles, so the true saturation current does NOT go to zero at tangency. Without it the characteristic has NO SOLUTION on a near-tangential surface. MEASURED on boundary type 5 (50.9 m^2 of far-SOL wall, 43.8 % near tangency): |j/j_sat| pinned at 24-27 while the characteristic can pass only 1 + s*X ~ 1.7, so u climbs without bound and the run dies at ~305 steps; type 1, where the field genuinely crosses the wall, runs 1350+. Scale: v_perp ~ D_perp/lambda_n, and with D_perp ~ 1 m^2/s, lambda_n ~ 1 cm that is ~100 m/s against c_s ~ 2.2e4 m/s at 5 eV, i.e. ~5e-3 of c_s - the same number sheath_min_bn was reproducing by hand as a gate. 0 = previous behaviour, bit-identical.
  logical :: sheath_weak_ufade    !< Fade the weak sheath residual to zero toward any node whose u is Dirichlet-frozen, and let the validity gate see the SHEATH-side excursion as well as the plasma-side one. A j-V relation is a statement about a surface free to respond; where u cannot move it is not a boundary condition but a contradiction. At a frozen node with u = 0 and V_wall = 0, X = -Lambda EXACTLY, so f = 1 - exp(Lambda) = -19.1 and the Gauss points near that node demand -19*j_sat of electron current. The existing guard skips writing a ROW on such a node but those Gauss points still enter the FREE neighbour's row through wk_F/wk_D/wk_S/tr_J, which are accumulated over the whole edge - so the neighbour is dragged onto the electron branch. MEASURED on type 4: area-averaged demand +1.66*j_sat (ion side, X should rise) yet ePhi/kTe goes 2.91 -> 0.47 -> -0.47 -> -5.59 with e-limited reaching 100 % of the area by step 3, and step 2's minimum (-4.01) lies BELOW the frozen node's own 0.05 - free nodes overshooting past the clamp, the signature of being dragged rather than of the plasma over-driving the sheath. No sheath parameter reaches it: sheath_zj_ratio_max builds its ratio from |zj0/zj_sat| = 2.39, the plasma demand, and never sees |zj_sh/zj_sat| = 19, which is why W reported 1.000 while the row was being poisoned. The fade VARIES within a row, so unlike a uniform weight it does not cancel out of F_a/D_a. Default .false. = previous behaviour, bit-identical.
  real*8  :: sheath_dfdx_min     !< Quasi-Newton floor on df/dX in the sheath characteristic. It regularises the potential/temperature columns when the physical slope becomes small, while leaving f and therefore the converged fixed point unchanged. It is not a strict delta-X bound for the fully coupled replacement row. Near the floating potential df/dX=1, so a floor below 1 is inactive. 0 = exact slope, bit-identical.
  logical :: sheath_jsat_from_vpar !< Build the ion saturation current from the parallel velocity the solution actually has, instead of from the Mach 1 value. zj_sat = c_sat*rho*g(b_n)*cs/|B| assumes v_par = c_s at the wall, which is exactly what bcs(...)%mach1 imposes, so the two are consistent by construction - but a detaching target is SUB-SONIC and the Mach 1 value then overestimates the ion flux, and with it j_sat. Switching this on swaps g(b_n)*cs/|B| for Vpar itself (same units: Vpar = v_par/|B|), so j_sat follows the real flux and inherits whatever drift correction Mach1BC carries. Floored by sheath_jsat_vpar_min, because zj_sat -> 0 makes the characteristic unsolvable. .false. = Mach 1 value (previous behaviour)
  real*8  :: sheath_jsat_vpar_min !< Floor on |Vpar| used by sheath_jsat_from_vpar, as a FRACTION of the Mach 1 value. Also catches flow directed away from the wall, where there is no ion flux to the sheath at all but zj_sat = 0 has no solution. A hard clip, so it drops its own sensitivity in the Jacobian, like the rho and T floors
  ...
  real*8  :: sheath_weak_rmax     !< Trust region on the weak sheath residual, in units of the local ion saturation current. zj_sh - zj is UNBOUNDED on the electron branch: at u = 0 the characteristic sits at X = -Lambda and f = 1-exp(Lambda) ~ -19, so a restart whose wall is not already near the floating potential asks the penalty for ~20*j_sat of driving force on the first step, which blows up. The residual is replaced by r/(1+|r|/(rmax*j_sat)), which is the identity for |r| << rmax*j_sat - so the fixed point and the local convergence rate are untouched - and saturates at rmax*j_sat however far off the characteristic the state starts. <= 0 disables the bound
  real*8  :: sheath_weak_relax    !< Under-relaxation of the weak sheath row: the replacement row enforces zj_new = zj_old + relax*(zj_sh_linearised - zj_old) instead of the full step, by scaling the state-variable columns AND the residual by relax while leaving the zj column (and hence the D_a normalisation) untouched. This is the weak-route analogue of the nodal szj_rel, and it is the fix for a PERIOD-2 step-to-step alternation: the sheath relation is stiff and quasi-Newton (the psi dependence through |B| and g_bn is deliberately omitted), so the effective gain can exceed 1 and the state flips between two branches each step - measured at 568+, I_wall alternating +-2% and e-limited alternating 5.1/6.0%, with the weak residual drifting 1e-2 -> 3e-2. 1 = full step, no relaxation, identical to the behaviour before this parameter existed. MEASURED 2026-08-26 AND NOT RECOMMENDED: that alternation was a transient of the relaxation phase, not an instability - it vanished on its own by step 3900 at relax = 1. Head-to-head at timeout, relax = 0.5 was worse on every metric: weak residual 6.3e-3 against 6.5e-4, Gauss residual 1.5e-2 against 9.0e-3, target closure 0.05-0.12 % against 0.00 %, ePhi/kTe max 7.32 and rising against 6.56 and falling, and 21 % more time in construct_matrix. Under-relaxation makes the row LAG a state that is still slowly evolving, so it never catches up. Kept as insurance against a genuine over-correction; leave it at 1
  real*8  :: sheath_weak_beta     !< DEPRECATED. Controlled a penalty added to the zj equation, which cannot work here: that equation is missing its surface term once dirichlet%zj is released, so adding to it cannot repair it, and a Galerkin trace block spans ~1e6 internally between value and derivative rows so a uniform coefficient wrecks the conditioning. Superseded by the row replacement in mod_sheath_trace, which normalises each row by its own trace mass diagonal and needs no parameter. > 0 is refused at setup
  real*8  :: T_min_sheath         !< Separate positivity floor for the temperatures the SHEATH characteristic sees, mirroring T_min_eta. The global T_min_neg is usually set well above divertor temperatures (T_min_neg = 3e-5 puts the knee at 1.93 eV), which compresses a real 0.3-1.5 eV target profile to 1.14-1.58 eV and flattens the radial gradient of Phi = Lambda*Te/e by ~3.5x - and that gradient IS the E_r driving the PFR ExB drift. < 0 = use T_min_neg, i.e. no change
  real*8  :: sheath_diag_R_split  !< Major radius dividing the INNER (R < split) from the OUTER (R >= split) target in the SHEATH diagnostic. Boundary type 1 carries both, so every total averages them together and opposite errors on the two targets cancel - useless for HFSHD, which IS an in-out difference. Set to about the X-point major radius. 0 = no split
  ...
  type type_bcs                           
    ...
    logical                  :: sheath_zj_weak !< Impose the sheath characteristic WEAKLY, as a penalty integrated over the boundary Gauss points, instead of as a nodal row. Measured motivation: with the nodal constraint |zj-zj_sh|/|zj_sat| is 1.8e-2 at the NODES and 3.9 at the GAUSS POINTS - a factor 220. The row is satisfied exactly where it is imposed and violated where the currents are integrated, which is why I_Ampere and I_sheath never closed however the characteristic, the gates or the relaxation were tuned. Enforcing it where it is measured removes that gap by construction. Use with dirichlet%zj = .false. and sheath_zj = .false. so no row replacement competes
  end type type_bcs
  ...
  real*8  :: T_min_eta            !< Do not use larger resistivity / hyper-resistivity values below this temperature (floor of the eta(T) and eta_num(T) dependencies); defaults to T_min; JOREK units. CAUTION, and initialise_parameters prints the number that actually applies: resistivity() is handed T_corr, which the caller has ALREADY passed through corr_neg_temp1, and that asymptotes to L1 = T_min_neg*corr_neg_temp_coef(1) from above. The clamp here keys on the RAW temperature and only zeroes the derivative, so whenever T_min_eta < L1 it cannot change the eta VALUE at all and the floor is set by T_min_neg through corr_neg instead. MEASURED on the AUG 38773 namelist (T_min = 1e-5, T_min_neg = 3e-5): L1 = 1.5e-5 = 0.73 eV sits ABOVE T_min, so eta is already capped at 17.2x its 4.9 eV value and T_min_eta is inert there - against a true 30.7x at 0.5 eV and 66x at 0.3 eV, i.e. a shortfall of only 1.8-3.8x rather than the 11x quoted for the older T_min = 1e-4 setup
