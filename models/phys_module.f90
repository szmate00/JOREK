...
module phys_module
  ...
  logical :: mach1_exb_term       !< Include the ExB term factor/Btot*R^2*u_b/ps0_b in the Mach-1
                                  ...
                                  !< along the wall.
  logical :: floating_keep_u_row  !< Keep the floating constraint in the u row even when
                                  !< dirichlet%w is false. Without it, clearing dirichlet%w also
                                  !< swaps the constraint into the w row, so "constraint in u,
                                  !< w free" cannot be selected and the two changes are confounded.
  ...
  logical :: grid_leg_wall_match !< Distribute the WALL points along a divertor leg with the same poloidal packing as the flux-aligned leg nodes they are paired with (grid_xpoint_wall, i.e. xpoint + grid_to_wall). By default the flux-aligned nodes use meshac2 with SIG_leg_0/SIG_leg_1 while the wall points are spread UNIFORMLY, so wherever the two distributions disagree the extension lines - which connect flux node j to wall point j - fan out and skew. The resulting cells are both thin and sheared, worst at the strike points where the flux-surface-to-wall gap also collapses. That is harmless with u = 0 at the wall, but any boundary condition imposing a gradient sees it immediately. Note grid_xpoint_wall already computes a packed distribution for the wall points and then discards it, which suggests this was intended and never wired up
