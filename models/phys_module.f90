...
module phys_module
  ...
  real*8  :: sheath_weak_beta     !< Penalty coefficient for bcs%sheath_zj_weak. SCALING: the zj equation is assembled as -(v_x*ps0_x + v_y*ps0_y + v*zj0)/BigR*xjac, so its diagonal goes as h^2/R, while this boundary term goes as beta*R*h. Their ratio is beta*R^2/h, i.e. PARITY is at beta ~ h/R^2 - about 1.5e-3 for a 3 mm boundary mesh at R = 1.4 m, NOT the O(1/h) that a bare Nitsche estimate suggests. Useful enforcement sits one to three decades above parity, so ~1e-2 is a sensible start. Far above that the penalty swamps zj = Delta*psi and becomes a Gauss-point Dirichlet with an effective zbig of beta*R^2/h - beta = 1e3 gives 6.5e5 and blows up in a couple of steps. Scan it and check the answer does not move
