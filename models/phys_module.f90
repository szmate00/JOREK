...
module phys_module
  ...
  logical :: floating_u_value_only !< Impose the floating potential on the NODE VALUE of u only, leaving u's tangential-derivative degree of freedom to the vorticity equation. That derivative row otherwise slaves du/dl to dTe/dl, and du/dl is the along-wall electric field, so grid-scale noise in the boundary temperature is converted directly into E, into ExB flow, and into vorticity - w = Delta*u amplifies it further. With this on, u still equals the floating potential AT EVERY NODE, so the physical variation of Phi with Te along the target is kept; only the interpolation between nodes is left to the equation, which has dissipation
