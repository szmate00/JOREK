...
module phys_module
  ...
  real*8  :: sheath_X_min         !< Legacy limiter parameter; does not alter the forward wall law (electron saturation is at Phi=Vwall).
  real*8  :: sheath_zj_ratio_max  !< Legacy nodal current-demand gate. Must be zero for weak trace replacement; solution-dependent row deletion is unsupported.
  ...
  real*8  :: sheath_sat_slope     !< Deprecated artificial ion super-saturation tail. Must be zero for the forward wall law.
  real*8  :: sheath_sat_slope_e   !< Deprecated artificial electron super-saturation tail. Must be zero for the forward wall law.
  real*8  :: sheath_weak_wmin    !< Deprecated solution-dependent weak-row gate. Must be zero.
  real*8  :: sheath_weak_detmin  !< Legacy static frame exclusion threshold. Nonzero freezes u and zj on excluded nodes; containment, not a mesh repair. Default zero.
  logical :: sheath_psi_jacobian  !< Include tangential and transverse psi columns through g(B.n/|B|) and |B| in weak current and Mach moments. Default true.
  real*8  :: sheath_v_perp       !< Deprecated signed parallel-speed floor, not a normal cross-field collection flux. Must be zero for the forward wall law.
  logical :: sheath_weak_ufade    !< Legacy fixed spatial weighting next to frozen-u/frame-excluded nodes; changes the weak test space. Default false.
  real*8  :: sheath_dfdx_min     !< Deprecated artificial current-law Jacobian slope floor. Must be zero; not a coupled trust region.
  logical :: sheath_jsat_from_vpar !< Use outgoing solved Vpar for ion collection only; electron reference remains the prescribed Bohm/Chodura state. Zero Vpar is a real state.
  real*8  :: sheath_jsat_vpar_min !< Explicit legacy ion-speed floor as a fraction of the Bohm/Chodura speed. Default zero; nonzero adds artificial ion collection.
  real*8  :: sheath_smooth_dX     !< Legacy soft-limiter width; unused by the forward current law.
  real*8  :: sheath_min_bn        !< Incidence threshold for diagnostics on the weak route; DOES NOT stabilize its normalized rows. Natural-u route retains its legacy gate.
  ...
  real*8  :: sheath_weak_rmax     !< Deprecated quadrature-residual clip; changes Galerkin roots. Must be zero on the weak route.
  real*8  :: sheath_weak_relax    !< Quasi-Newton relaxation of weak current state columns and RHS, leaving its current mass block unchanged. Default one; not an accepted-step line search.
  real*8  :: sheath_weak_beta     !< Deprecated penalty coefficient, must be zero. Weak current/Mach constraints use exact row clearing and max-coefficient row scaling.
  real*8  :: T_min_sheath         !< Sheath and weak Mach temperature map: negative uses T_min_neg, otherwise a separate positive floor. Nondefault values can differ from transport/heat closures.
