...
module phys_module
  ...
  real*8  :: sheath_dfdx_min     !< Levenberg-Marquardt floor on df/dX in the sheath characteristic. The trust region sheath_weak_rmax bounds the RESIDUAL at rmax*j_sat, but the Newton step is F/J and nothing bounds it when J = df/dX collapses - which it does on both plateaus: df/dX -> s on the ion side and -> 0 through sheath_x_limited on the electron side. A capped residual over a vanishing slope is an unbounded step, and that is the observed one-step jump |j/jsat| 15 -> 330 -> 5089 with everything healthy the step before. Flooring the JACOBIAN bounds the step to |dX| <= rmax/dfdx_min while leaving f, and therefore the fixed point, exactly untouched - unlike sheath_sat_slope_e, which modifies f itself and does move the solution. Near the floating potential df/dX = 1, so any floor below 1 is inactive there and genuine sheath physics is preserved. 0 = off, bit-identical.
