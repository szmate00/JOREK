...
subroutine preset_parameters
  ...
  sheath_j_min_angle    = -1.d0   ! < 0: the sheath current row uses min_sheath_angle; set e.g. 3 to keep it off grazing corner cells without touching the flux floors
  floating_u_prof_every = 0       ! [wall prof] per-Gauss-point wall profile every N steps (0 = off)
  sheath_j_release_der  = .true.  ! .f.: only the value DOF of u released at sheath nodes; derivative DOFs stay on the floating row (kills the sub-element sawtooth of dPhi/ds measured 2026-09-27)
  sheath_Te_smooth      = 0       ! passes of tangential Te smoothing for the wall rows (0 = the raw wall Te)
