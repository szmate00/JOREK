...
!! Rebuilt before each weak-sheath matrix construction, including restart and PC
...
module mod_sheath_boundary_edges
  implicit none
  private
  public :: sheath_edges_build, sheath_edge_is_exterior
  logical, allocatable, save :: exterior(:,:)
contains
  subroutine sheath_edges_build(element_list, node_list)
    ...
    do e=1,ne
      ...
      do s=1,4
        ...
        if (a <= 0 .or. b <= 0) then
          write(*,*) 'ERROR: weak sheath requires assigned global vertex DOFs.'
        ...
        if (counts(slot) > 2) then
          write(*,*) 'ERROR: non-manifold edge in weak sheath connectivity: ',a,b
          call MPI_Abort(MPI_COMM_WORLD,1,ierr)
          return
        endif
      enddo
    ...
    enddo
  end subroutine sheath_edges_build

  logical function sheath_edge_is_exterior(element_id, side)
    integer, intent(in) :: element_id, side
    sheath_edge_is_exterior = exterior(side,element_id)
  end function sheath_edge_is_exterior
end module mod_sheath_boundary_edges
